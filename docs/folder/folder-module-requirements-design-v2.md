...
| level | int | 层级深度，根为 0 |
| sortNo | int | 排序号 |
| ownerDeptId | bigint | 所属部门 ID |
| ownerUserId | bigint | 负责人 / 创建人 |
...
- **Sa-Token 负责“你是不是系统管理员”**
- **FolderPermissionService 负责“你能不能操作这个目录”**
...
1. `loadRootTree`：返回从根开始的可见树
2. `loadChildren(parentId)`：懒加载子节点
...
2. `FOLDER_ADMIN` 是否允许对所有目录做授权变更  
   - 我建议允许，但根节点相关操作仍受限

3. 复制目录时，是否保留 `ownerDeptId / ownerUserId`  
   - 我建议保留 `ownerDeptId`，`ownerUserId` 改为当前操作人
...
5. **先把树、授权、收藏、审计、查询这些 folder 自己能闭环的能力做扎实**
